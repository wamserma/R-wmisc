...
test_that("new tokenizer has default delimiters",{
  expect_warning(tok<-Tokenizer$new())
  expect_identical(tok$get_delimiters(),c(9,10,13,32))
})

test_that("modifying delimiters works",{
  expect_warning(tok<-Tokenizer$new())
  tmp<-tok$get_delimiters()
  tmp[3]<-42
  tok$set_delimiters(tmp)
  expect_identical(tok$get_delimiters(),c(9,10,42,32))
})

test_that("Only ASCII-characters (numerical value < 256) accepted.",{
  expect_warning(tok<-Tokenizer$new())
  expect_error(tok$set_delimiters(c(3,4,5,12345)),"Only ASCII.*")
})
